...
let html_content = `
  ...
  <script>
    ...
    function ${thisMacroName()}_reloadTokencItem(actorId,citemname){ 
      let actor;  
      let token; 
      ...
      if(actor!=null){          
        ...
        actor.sheet.rechargeCI(citem.id, citem.ciKey);
        console.debug("Item", citem);
      }
    }
              ...
              if (citem.groups.length>0){                         
                ...
                for(let iGroup=0;iGroup<citem.groups.length;iGroup++){  
                  ...
                  if(citem.groups[iGroup].ikey==weaponSelectedGroupKey){ 
                    ...
                    lielement.innerHTML='<label class="${thisMacroName()}_hbo" onclick="${thisMacroName()}_reloadTokencItem(' + ${thisMacroName()}_singleQuoted(actorId) +','+ ${thisMacroName()}_singleQuoted(citem.name) + ');">' + citem.name + '</label> ';
